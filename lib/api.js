...
export const connectionAPI = {
  ...
  resync: async (connectionId) => {
    try {
      const response = await api.post(`/connections/${connectionId}/resync`);
      return response.data;
    } catch (error) {
      console.error("Error resyncing connection:", error);
      return {
        status: "error",
        message: error.response?.data?.message || "Failed to resync connection",
      };
    }
  },
};
...
export const queryAPI = {
  ...
  executeQuery: async (connectionId, query, pairs = [],conversation_id=null) => {
...
export const dashboardAPI = {
  ...
  updateLayout: async (dashboardId, layout) => {
    try {
      console.log(`Updating layout for dashboard ${dashboardId}:`, layout);
      const response = await api.put(`/dashboards/${dashboardId}/layout`, layout);
      console.log('Layout update response:', response.data);
      return response.data;
    } catch (error) {
...
export const conversationAPI = {
  getConversations: async (connectionId) => {
    try {
      console.log('Fetching conversations...');
      const response = await api.get(`/conversations/connection/${connectionId}`);
      return {
        status: 'success',
  ...
  createConversation: async (title, connectionId) => {
    try {
  ...
  sendMessage: async (conversationId, content, connectionId, pairs = []) => {
    try {
