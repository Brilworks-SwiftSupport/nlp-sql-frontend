  ...
  updateConnection: async (connectionId, connectionData) => {
    ...
    const payload = {
      ...
      ssh_config: connectionData.ssh_enabled ? {
        ...
        ...(connectionData.ssh_password && { password: connectionData.ssh_password }),
        ...(connectionData.ssh_private_key && { private_key: connectionData.ssh_private_key })
      } : null
    };
...
export const queryAPI = {
  ...
  executeQuery: async (connectionId, query, pairs = []) => {
    ...
    const queryPromise = (async () => {
      try {
        const response = await api.post(`/queries/execute/${connectionId}`, { query, pairs });
        return response.data;
      } finally {
...
export const dashboardAPI = {
  ...
  getDashboardsByCollection: async (connectionId) => {
    try {
      const response = await api.get(`/dashboards/${connectionId}`);
      return {
        status: 'success',
        dashboards: response.data.dashboards || []
      };
      ...
      return {
        status: 'error',
        message: error.response?.data?.message || 'Failed to fetch dashboards for this collection',
        dashboards: []
      };
    }
  },
